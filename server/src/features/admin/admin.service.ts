...
export class AdminService {
	...
	async getSystemStatistics(period?: string) {
		...
		const [
			...
			this.postRepository.count({
				where: { createdAt: MoreThanOrEqual(startDate), status: 'published' },
			}),
			...
			this.postRepository.count({
				where: { createdAt: MoreThanOrEqual(this.getTodayStart()), status: 'published' },
			}),
	...
	async getTopContent() {
		const [topPosts, topCourses] = await Promise.all([this.getTopPosts(), this.getTopCourses()])
	...
	private async getDbConnectionsCount(): Promise<number> {
		...
		const activityConnections = Math.min(
			10,
			Math.floor((recentUsers + recentPosts + recentComments) / 2)
		)
